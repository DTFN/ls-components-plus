import { createSeriesData_default, prepareSeriesDataSchema } from './chunk-NEWCSECF.js';
import {
  LineDraw_default,
...
import { getLabelLineStatesModels, limitSurfaceAngle, limitTurnAngle, setLabelLineStyle } from './chunk-GI2ZEGHQ.js';
import {
  BezierCurve_default,
  Circle_default,
  DISPLAY_STATES,
          ...
          each(coordSys.getAxes(), function (axis, idx) {
            if (axis.type === 'category' && dims != null) {
              ...
              for (var i = 0; i < tickCoords.length; i++) {
                ...
                var tickValue =
                  i === tickCoords.length - 1 ? tickCoords[i - 1].tickValue + stepTickValue : tickCoords[i].tickValue;
...
function radarLayout(ecModel) {
  ecModel.eachSeriesByType('radar', function (seriesModel) {
    ...
    data.each(function (idx) {
      var firstPoint =
        find(points[idx], function (point) {
        ...
        for (var i = 0; i < newPoints.length - 1; i++) {
          ...
          if (symbolPath) {
            ...
            if (oldPoints[i]) {
              ...
              graphic_exports[isInit ? 'initProps' : 'updateProps'](
                symbolPath,
                {
                  x: newPoints[i][0],
                  y: newPoints[i][1]
...
function apportion(subtreeV, subtreeW, ancestor, separation2) {
  if (subtreeW) {
    ...
    while (((nodeInLeft = nextRight(nodeInLeft)), (nodeInRight = nextLeft(nodeInRight)), nodeInLeft && nodeInRight)) {
      ...
      var shift =
        nodeInLeft.hierNode.prelim + sumInLeft - nodeInRight.hierNode.prelim - sumInRight + separation2(nodeInLeft, nodeInRight);
      if (shift > 0) {
...
function updateNode(data, dataIndex, symbolEl, group, seriesModel) {
  ...
  var focusDataIndices =
    focus === 'relative'
      ? concatArray(node.getAncestorsIndices(), node.getDescendantIndices())
      : focus === 'ancestor'
        ? node.getAncestorsIndices()
        : focus === 'descendant'
          ? node.getDescendantIndices()
          : null;
  if (focusDataIndices) {
...
var TreemapView =
  ...
  (function (_super) {
    ...
    TreemapView2.prototype.render = function (seriesModel, ecModel, api, payload) {
      ...
      hasAnimation && !isInit && (!payloadType || payloadType === 'treemapZoomToNode' || payloadType === 'treemapRootToNode')
    ...
    TreemapView2.prototype._doRender = function (containerGroup, seriesModel, reRoot) {
      ...
      function dualTravel(thisViewChildren, oldViewChildren, parentGroup, sameTree, depth) {
        if (sameTree) {
          ...
          new DataDiffer_default(oldViewChildren, thisViewChildren, getKey2, getKey2)
            .add(processNode)
            .update(processNode)
      ...
      function clearStorage(storage) {
        ...
        storage &&
          each(storage, function (store, storageName) {
    ...
    TreemapView2.prototype._renderBreadcrumb = function (seriesModel, api, targetInfo) {
      ...
      (this._breadcrumb || (this._breadcrumb = new Breadcrumb_default(this.group))).render(
        seriesModel,
        api,
        targetInfo.node,
        function (node) {
...
function renderNode(
  ...
  var focusOrIndices =
    focus === 'ancestor' ? thisNode.getAncestorsIndices() : focus === 'descendant' ? thisNode.getDescendantIndices() : focus;
  if (isParent) {
...
function prunning(node, clipRect, viewAbovePath, viewRoot, depth) {
  ...
  var childClipRect = new BoundingRect_default(
  ...
  (function (_super) {
    ...
    GaugeView2.prototype._renderAnchor = function (seriesModel, posInfo) {
      ...
      if (showAnchor) {
        ...
        var anchor = createSymbol(
          anchorType,
          posInfo.cx - anchorSize / 2 + parsePercent2(offsetCenter[0], posInfo.r),
          posInfo.cy - anchorSize / 2 + parsePercent2(offsetCenter[1], posInfo.r),
          anchorSize,
          anchorSize,
          null,
          anchorKeepAspect
    ...
    GaugeView2.prototype._renderTitleAndDetail = function (seriesModel, ecModel, api, getColor, posInfo) {
      ...
      data.each(function (idx) {
        ...
        if (itemDetailModel.get('show')) {
          ...
          hasAnimation &&
            animateLabelValue(labelEl, idx, data, seriesModel, {
...
function funnelLayout(ecModel, api) {
  ecModel.eachSeriesByType('funnel', function (seriesModel) {
    ...
    var sizeExtent =
      orient === 'horizontal'
        ? [parsePercent2(seriesModel.get('minSize'), viewHeight), parsePercent2(seriesModel.get('maxSize'), viewHeight)]
        : [parsePercent2(seriesModel.get('minSize'), viewWidth), parsePercent2(seriesModel.get('maxSize'), viewWidth)];
  ...
  (function (_super) {
    ...
    SankeyView2.prototype.render = function (seriesModel, ecModel, api) {
      ...
      if (!this._data && seriesModel.isAnimationEnabled()) {
        group.setClipPath(
          createGridClipShape2(group.getBoundingRect(), seriesModel, function () {
            group.removeClipPath();
          })
        );
      }
...
var EffectLine =
  ...
  (function (_super) {
    ...
    EffectLine2.prototype._updateSymbolPosition = function (symbol) {
      ...
      var tx =
        symbol.__t < 1 ? quadraticDerivativeAt2(p1[0], cp1[0], p2[0], t) : quadraticDerivativeAt2(p2[0], cp1[0], p1[0], 1 - t);
      var ty =
        symbol.__t < 1 ? quadraticDerivativeAt2(p1[1], cp1[1], p2[1], t) : quadraticDerivativeAt2(p2[1], cp1[1], p1[1], 1 - t);
    ...
    LinesView2.prototype._updateLineDraw = function (data, seriesModel) {
      ...
      if (!lineDraw || hasEffect !== this._hasEffet || isPolyline !== this._isPolyline || isLargeDraw !== this._isLargeDraw) {
        ...
        lineDraw = this._lineDraw = isLargeDraw
          ? new LargeLineDraw_default()
          : new LineDraw_default(
              isPolyline
                ? hasEffect
                  ? EffectPolyline_default
                  : Polyline_default2
                : hasEffect
                  ? EffectLine_default
                  : Line_default2
  ...
  (function (_super) {
    ...
    HeatmapView2.prototype._renderOnCartesianAndCalendar = function (seriesModel, api, start, end, incremental) {
      ...
      var dataDims = isCartesian2d
        ? [data.mapDimension('x'), data.mapDimension('y'), data.mapDimension('value')]
...
function prepareSymbolSize(
  ...
  parsedSymbolSize[valueDim.index] = parsePercent2(
...
function prepareLayoutInfo(
  ...
  pathPosition[valueDim.index] =
    symbolPosition === 'start' ? sizeFix : symbolPosition === 'end' ? boundingLength - sizeFix : boundingLength / 2;
  if (symbolOffset) {
...
function createOrUpdateClip(bar, opt, symbolMeta, isUpdate) {
  if (symbolMeta.symbolClip) {
    ...
    if (clipPath) {
      ...
      graphic_exports[isUpdate ? 'updateProps' : 'initProps'](
        clipPath,
        {
          shape: target
        },
...
var ThemeRiverView =
  ...
  (function (_super) {
    ...
    ThemeRiverView2.prototype.render = function (seriesModel, ecModel, api) {
      ...
      var newLayersGroups = [];
      dataDiffer
        .add(bind(process2, this, 'add'))
        .update(bind(process2, this, 'update'))
        .remove(bind(process2, this, 'remove'))
        .execute();
      function process2(status, idx, oldIdx) {
        ...
        if (status === 'add') {
          ...
          if (seriesModel.isAnimationEnabled()) {
            polygon.setClipPath(
              createGridClipShape3(polygon.getBoundingRect(), seriesModel, function () {
...
var SunburstPiece =
  ...
  (function (_super) {
    ...
    SunburstPiece2.prototype.updateData = function (firstCreate, node, seriesModel, ecModel, api) {
      ...
      var focus = emphasisModel.get('focus');
      var focusOrIndices =
        focus === 'ancestor' ? node.getAncestorsIndices() : focus === 'descendant' ? node.getDescendantIndices() : focus;
  ...
  (function (_super) {
    ...
    SunburstView2.prototype.render = function (seriesModel, ecModel, api, payload) {
      ...
      function dualTravel(newChildren2, oldChildren2) {
        ...
        new DataDiffer_default(oldChildren2, newChildren2, getKey2, getKey2)
          .add(processNode)
          .update(processNode)
          .remove(curry(processNode, null))
          .execute();
...
var SunburstSeriesModel =
  ...
  (function (_super) {
    ...
    SunburstSeriesModel2.prototype.getInitialData = function (option, ecModel) {
      ...
      var levelModels = (this._levelModels = map(
        option.levels || [],
        function (levelDefine) {
    ...
    return !data.hasItemOption
      ...
      : dataIndexInside === currDataIndexInside
        ? currItemStyleModels[state] ||
          (currItemStyleModels[state] = getItemModel2(dataIndexInside).getModel(PATH_ITEM_STYLE[state]))
        : getItemModel2(dataIndexInside).getModel(PATH_ITEM_STYLE[state]);
  }
  function getLabelModel(dataIndexInside, state) {
    return !data.hasItemOption
      ? seriesLabelModels[state]
      : dataIndexInside === currDataIndexInside
        ? currLabelModels[state] || (currLabelModels[state] = getItemModel2(dataIndexInside).getModel(PATH_LABEL[state]))
        : getItemModel2(dataIndexInside).getModel(PATH_LABEL[state]);
  }
...
function doesElNeedRecreate(el, elOption, seriesModel) {
  ...
  return (
    ...
    (elOptionType === 'path' && hasOwnPathData(elOptionShape) && getPathData(elOptionShape) !== elInner.customPathData) ||
    (elOptionType === 'image' && hasOwn(elOptionStyle, 'image') && elOptionStyle.image !== elInner.customImagePath)
  );
}
...
function processTxInfo(elOption, state, attachedTxInfo) {
  ...
  if (
    styleOpt && // Because emphasis style has little info to detect legacy,
...
function diffGroupChildren(context) {
  new DataDiffer_default(context.oldChildren, context.newChildren, getKey, getKey, context)
