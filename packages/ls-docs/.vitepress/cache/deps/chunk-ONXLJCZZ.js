...
function preparePointerTransformer(markers, saved, inverse) {
  ...
  return oldCoordTheSame && transformer
    ...
    : ((saved.srcCoords = srcCoords),
      (saved[transformerName] = inverse ? buildTransformer(destCoords, srcCoords) : buildTransformer(srcCoords, destCoords)));
}
...
function parseRichText(text, style) {
  ...
  var overflow = style.overflow;
  var wrapInfo =
    (overflow === 'break' || overflow === 'breakAll') && topWidth != null
  ...
  for (var i = 0; i < len2; ) {
    ...
    switch (cmd) {
      ...
      case CMD2.R:
        ...
        if (isStroke) {
          if (
            containStroke(x0, y0, x1, y0, lineWidth, x, y) ||
...
var ZRText = (function (_super) {
  ...
  ZRText2.prototype._updatePlainTexts = function () {
    ...
    var fixedBoundingRect =
      style.width != null && (style.overflow === 'truncate' || style.overflow === 'break' || style.overflow === 'breakAll');
    ...
    for (var i = 0; i < textLines.length; i++) {
      ...
      if (fixedBoundingRect) {
        el.setBoundingRect(
          new BoundingRect_default(
            adjustTextX(subElStyle.x, style.width, subElStyle.textAlign),
            adjustTextY2(subElStyle.y, calculatedLineHeight, subElStyle.textBaseline),
            contentWidth,
            calculatedLineHeight
          )
        );
  ...
  ZRText2.prototype._placeToken = function (token, style, lineHeight, lineTop, x, textAlign, parentBgColorDrawn) {
    ...
    needDrawBg &&
      this._renderBackground(
        tokenStyle,
        style,
        textAlign === 'right' ? x - token.width : textAlign === 'center' ? x - token.width / 2 : x,
        y - token.height / 2,
        token.width,
        token.height
      );
    ...
    var textFill = getFill(
      'fill' in tokenStyle ? tokenStyle.fill : 'fill' in style ? style.fill : ((useDefaultFill = true), defaultStyle.fill)
    );
    var textStroke = getStroke(
      'stroke' in tokenStyle
        ...
        : 'stroke' in style
          ? style.stroke
          : !bgColorDrawn && !parentBgColorDrawn && (!defaultStyle.autoStroke || useDefaultFill)
            ? ((defaultLineWidth = DEFAULT_STROKE_LINE_WIDTH), defaultStyle.stroke)
            : null
    );
    ...
    el.setBoundingRect(
      new BoundingRect_default(
        ...
        adjustTextY2(subElStyle.y, textHeight, subElStyle.textBaseline),
        textWidth,
        textHeight
      )
    );
...
tslib/tslib.es6.js:
  ...
  ***************************************************************************** *)
*/
//# sourceMappingURL=chunk-ONXLJCZZ.js.map
