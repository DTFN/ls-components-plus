...
import {
  ...
  warn,
  windowOpen
} from './chunk-NFXVF2BT.js';
import { addEventListener, normalizeEvent, stop } from './chunk-RGQRN4HP.js';
import {
  ...
  (function () {
    ...
    BaseAxisPointer2.prototype._moveHandleToValue = function (value, isInit) {
      updateProps2(
        ...
        this._handle,
        getHandleTransProps(this.getHandleTransform(value, this._axisModel, this._axisPointerModel))
      );
    };
...
function getTransformedPosition(axis, value, layoutInfo) {
  ...
  return applyTransform(
    [axis.dataToCoord(value), (layoutInfo.labelOffset || 0) + (layoutInfo.labelDirection || 1) * (layoutInfo.labelMargin || 0)],
    transform
  );
}
        ...
        if (srcAxisInfo !== tarAxisInfo && srcValItem) {
          ...
          linkGroup.mapper &&
            (val = tarAxisInfo.axis.scale.parse(
              linkGroup.mapper(val, makeMapperParam(srcAxisInfo), makeMapperParam(tarAxisInfo))
            ));
...
function barLayoutPolar(seriesType, ecModel, api) {
  ...
  var barWidthAndOffset = calRadialBar(
    filter(ecModel.getSeriesByType(seriesType), function (seriesModel) {
      return (
        !ecModel.isSeriesFiltered(seriesModel) && seriesModel.coordinateSystem && seriesModel.coordinateSystem.type === 'polar'
      );
    })
  );
...
function calRadialBar(barSeries) {
  ...
  each(barSeries, function (seriesModel, idx) {
    ...
    var bandWidth =
      baseAxis.type === 'category' ? baseAxis.getBandWidth() : Math.abs(axisExtent[1] - axisExtent[0]) / data.count();
...
var Single =
  ...
  (function () {
    ...
    Single2.prototype._init = function (axisModel, ecModel, api) {
      ...
      var axis = new SingleAxis_default(
        dim,
  ...
  (function (_super) {
    ...
    CalendarView2.prototype._renderWeekText = function (calendarModel, localeModel, rangeData, orient, group) {
      ...
      if (!nameMap || isString(nameMap)) {
        ...
        nameMap =
          dayOfWeekShort ||
          map(localeModel.get(['time', 'dayOfWeekAbbr']), function (val) {
            return val[0];
          });
      }
...
function removeEl(elExisting, elOption, elMap, graphicModel) {
  ...
  if (existElParent) {
    elExisting.type === 'group' &&
      elExisting.traverse(function (el) {
    ...
    DataZoomModel2.prototype._fillAutoTargetAxisByOrient = function (targetAxisIndexMap, orient) {
      ...
      function setParallelAxis(axisModels2, axisDim2) {
        ...
        if (axisDim2 === 'x' || axisDim2 === 'y') {
          ...
          gridModel_1 &&
            each(axisModels2, function (axModel) {
              if (
...
var ToolboxView =
  ...
  (function (_super) {
    ...
    ToolboxView2.prototype.render = function (toolboxModel, ecModel, api, payload) {
      ...
      new DataDiffer_default(this._featureNames || [], featureNames)
        .add(processFeature)
...
function assembleSeriesWithCategoryAxis(groups) {
  ...
  each(groups, function (group, key) {
    ...
    var valueAxisDim = valueAxis.dim;
    var headers = [' '].concat(
      map(group.series, function (series) {
        return series.name;
      })
    );
    ...
    each(group.series, function (series) {
      ...
      columns.push(
        series.getRawData().mapArray(rawData.mapDimension(valueAxisDim), function (val) {
          return val;
        })
      );
    });
...
function getContentFromModel(ecModel) {
  ...
  return {
    value: filter(
      [assembleSeriesWithCategoryAxis(result.seriesGroupByCategoryAxis), assembleOtherSeries(result.other)],
      function (str) {
        return !!str.replace(/[\n\t\s]/g, '');
      }
    ).join('\n\n' + BLOCK_SPLITER + '\n\n'),
...
var gCssText =
  ...
  (env_default.transform3dSupported ? 'will-change:transform;' : '');
...
function assembleArrow(tooltipModel, borderColor, arrowPosition) {
  ...
  var arrowOffset =
    Math.round(((rotatedWH - Math.SQRT2 * borderWidth) / 2 + Math.SQRT2 * borderWidth - (rotatedWH - arrowWH) / 2) * 100) / 100;
...
function assembleTransition(duration, onlyFade) {
  ...
  if (!onlyFade) {
    ...
    transitionText += env_default.transformSupported
...
function assembleFont(textStyleModel) {
  ...
  shadowColor &&
    shadowBlur &&
    cssText.push('text-shadow:' + shadowOffsetX + 'px ' + shadowOffsetY + 'px ' + shadowBlur + 'px ' + shadowColor);
  ...
  (function (_super) {
    ...
    TooltipView2.prototype.init = function (ecModel, api) {
      ...
      this._tooltipContent =
        renderMode === 'richText'
          ? new TooltipRichContent_default(api)
          : new TooltipHTMLContent_default(api, {
    ...
    TooltipView2.prototype._initGlobalListener = function () {
      ...
      register(
        'itemTooltip',
        this._api,
        bind(function (currTrigger, e, dispatchAction3) {
          if (triggerOn !== 'none') {
    ...
    TooltipView2.prototype._manuallyAxisShowTip = function (tooltipModel, ecModel, api, payload) {
      ...
      var tooltipCascadedModel = buildTooltipModel(
        [data.getItemModel(dataIndex), seriesModel, (seriesModel.coordinateSystem || {}).model],
        this._tooltipModel
      );
    ...
    TooltipView2.prototype._showSeriesItemTooltip = function (e, dispatcher, dispatchAction3) {
      ...
      var tooltipModel = buildTooltipModel(
        [data.getItemModel(dataIndex), dataModel, seriesModel && (seriesModel.coordinateSystem || {}).model],
        this._tooltipModel,
        positionDefault
          ? {
        ...
        each(lastCoordSys, function (lastItemCoordSys, indexCoordSys) {
          ...
          contentNotChanged &&
            each(lastDataByAxis, function (lastItem, indexAxis) {
              ...
              lastCbParamsList &&
                each(lastItem.seriesDataIndices, function (idxItem) {
...
var selector = {
  ...
  polygon: {
    point: function (itemLayout, selectors, area) {
      return (
        itemLayout && area.boundingRect.contain(itemLayout[0], itemLayout[1]) && contain(area.range, itemLayout[0], itemLayout[1])
      );
    },
    rect: function (itemLayout, selectors, area) {
      ...
      if (
        contain(points, x, y) ||
        contain(points, x + width, y) ||
        contain(points, x, y + height) ||
        contain(points, x + width, y + height) ||
        BoundingRect_default.create(itemLayout).contain(p[0], p[1]) ||
  ...
  return {
    ...
    rect: function (itemLayout, selectors, area) {
      if (itemLayout) {
        ...
        return (
          inLineRange(layoutRange[0], range) ||
...
function brushVisual(ecModel, api, payload) {
  ...
  ecModel.eachComponent(
    ...
    function (brushModel, brushIndex) {
      ...
      isArray(brushLink) &&
        each(brushLink, function (seriesIndex) {
      ...
      function stepAParallel(seriesModel, seriesIndex) {
        ...
        linkOthers(seriesIndex) &&
          coordSys.eachActiveState(seriesModel.getData(), function (activeState, dataIndex) {
    ...
    BrushFeature2.prototype.render = function (featureModel, ecModel, api) {
      ...
      each(featureModel.get('type', true), function (type) {
        featureModel.setIconStatus(
          type,
          (type === 'keep' ? brushMode === 'multiple' : type === 'clear' ? isBrushed : type === brushType) ? 'emphasis' : 'normal'
        );
      });
    };
  ...
  (function (_super) {
    ...
    SliderTimelineView2.prototype._layout = function (timelineModel, api) {
      ...
      return {
        ...
        labelBaseline:
          timelineModel.get(['label', 'verticalAlign']) || timelineModel.get(['label', 'baseline']) || labelBaselineMap[orient],
        // Based on mainGroup.
        playPosition,
        prevBtnPosition,
        nextBtnPosition,
    ...
    SliderTimelineView2.prototype._renderControl = function (layoutInfo, group, axis, timelineModel) {
      ...
      function makeBtn(position, iconName, onclick, willRotate) {
        ...
        var iconSize = parsePercent(
          retrieve2(timelineModel.get(['controlStyle', iconName + 'BtnSize']), controlSize),
          controlSize
        );
...
function makeControlIcon(timelineModel, objPath, rect, opts) {
  ...
  var icon = createIcon(
    ...
    opts || {},
    new BoundingRect_default(rect[0], rect[1], rect[2], rect[3])
  );
  if (style) {
    icon.setStyle(style);
  }
...
function ifMarkLineHasOnlyDim(dimIndex, fromCoord, toCoord, coordSys) {
  ...
  return (
    isInfinity(fromCoord[otherDimIndex]) &&
    isInfinity(toCoord[otherDimIndex]) &&
...
function markLineFilter(coordSys, item) {
  if (coordSys.type === 'cartesian2d') {
    ...
    if (
      fromCoord &&
      toCoord &&
      (ifMarkLineHasOnlyDim(1, fromCoord, toCoord, coordSys) || ifMarkLineHasOnlyDim(0, fromCoord, toCoord, coordSys))
    ) {
      return true;
    }
  }
...
function markAreaFilter(coordSys, item) {
  ...
  if (isCoordinateSystemType(coordSys, 'cartesian2d')) {
    if (
      fromCoord &&
      toCoord &&
      (ifMarkAreaHasOnlyDim(1, fromCoord, toCoord, coordSys) || ifMarkAreaHasOnlyDim(0, fromCoord, toCoord, coordSys))
    ) {
      return true;
    }
...
var MarkAreaView =
  ...
  (function (_super) {
    ...
    MarkAreaView2.prototype.renderSeries = function (seriesModel, maModel, ecModel, api) {
      ...
      areaData
        .diff(inner10(polygonGroup).data)
        .add(function (idx) {
  ...
  (function (_super) {
      ...
      each7(
        ...
        function (legendItemModel, dataIndex) {
          ...
          if (seriesModel) {
            ...
            itemGroup
              .on('click', curry2(dispatchSelectAction, name, null, api, excludeSeriesId))
              .on('mouseover', curry2(dispatchHighlightAction, seriesModel.name, null, api, excludeSeriesId))
              .on('mouseout', curry2(dispatchDownplayAction, seriesModel.name, null, api, excludeSeriesId));
            if (ecModel.ssr) {
            ...
            ecModel.eachRawSeries(function (seriesModel2) {
              ...
              if (seriesModel2.legendVisualProvider) {
                ...
                itemGroup2
                  .on('click', curry2(dispatchSelectAction, null, name, api, excludeSeriesId))
                  .on('mouseover', curry2(dispatchHighlightAction, null, name, api, excludeSeriesId))
                  .on('mouseout', curry2(dispatchDownplayAction, null, name, api, excludeSeriesId));
                if (ecModel.ssr) {
    ...
    LegendView2.prototype._createItem = function (
      ...
      if (isFunction(seriesModel.getLegendIcon) && (!legendIconType || legendIconType === 'inherit')) {
        ...
        var rotate2 =
          legendIconType === 'inherit' && seriesModel.getData().getVisual('symbol')
            ? iconRotate === 'inherit'
...
function getLegendStyle(iconType, legendItemModel, lineVisualStyle, itemVisualStyle, drawType, isSelected, api) {
  ...
  itemStyle.decal =
    !decalStyle || decalStyle === 'inherit' ? itemVisualStyle.decal : createOrUpdatePatternFromDecal(decalStyle, api);
  ...
  if (!isSelected) {
    ...
    itemStyle.lineWidth =
      borderWidth === 'auto' ? (itemVisualStyle.lineWidth > 0 && visualHasBorder ? 2 : 0) : itemStyle.lineWidth;
...
var ScrollableLegendView =
  ...
  (function (_super) {
    ...
    ScrollableLegendView2.prototype._updatePageInfoView = function (legendModel, pageInfo) {
      ...
      each(['pagePrev', 'pageNext'], function (name) {
        ...
        if (icon) {
          icon.setStyle(
            'fill',
            canJump ? legendModel.get('pageIconColor', true) : legendModel.get('pageIconInactiveColor', true)
          );
      ...
      pageText &&
        ...
        pageText.setStyle(
          ...
          isString(pageFormatter)
            ? pageFormatter
                .replace('{current}', current == null ? '' : current + '')
    ...
    ScrollableLegendView2.prototype._getPageInfo = function (legendModel) {
      ...
      for (
        var i = targetItemIndex + 1, winStartItemInfo = targetItemInfo, winEndItemInfo = targetItemInfo, currItemInfo = null;
        i <= itemCount;
        ++i
      ) {
  ...
  (function (_super) {
    ...
    SliderZoomView2.prototype._renderHandle = function () {
      ...
      each(
        ...
        function (handleIndex) {
          ...
          thisGroup.add(
            (handleLabels[handleIndex] = new Text_default({
              silent: true,
              invisible: true,
    ...
    SliderZoomView2.prototype._updateInterval = function (handleIndex, delta) {
      ...
      sliderMove(
        ...
        minMaxSpan.minSpan != null ? linearMap(minMaxSpan.minSpan, percentExtent, viewExtend, true) : null,
        minMaxSpan.maxSpan != null ? linearMap(minMaxSpan.maxSpan, percentExtent, viewExtend, true) : null
      );
    ...
    SliderZoomView2.prototype._updateDataInfo = function (nonRealtime) {
      ...
      function setLabel(handleIndex) {
        ...
        var textPoint = applyTransform(
          [orderedHandleEnds[handleIndex] + (handleIndex === 0 ? -offset : offset), this._size[1] / 2],
          barTransform
        );
        ...
        each8(
          ...
          function (state) {
            ...
            if (visuals.symbolSize == null) {
              visuals.symbolSize =
                (symbolSizeExists && clone(symbolSizeExists)) || (isCategory ? itemSize[0] : [itemSize[0], itemSize[0]]);
            }
...
var ContinuousModel =
  ...
  (function (_super) {
    ...
    ContinuousModel2.prototype.getValueState = function (value) {
      var range = this.option.range;
      var dataExtent = this.getExtent();
      return (range[0] <= dataExtent[0] || range[0] <= value) && (range[1] >= dataExtent[1] || value <= range[1])
        ? 'inRange'
        : 'outOfRange';
    };
    ...
    ContinuousView2.prototype._renderBar = function (targetGroup) {
      ...
      gradientBarGroup.add(
        (shapes.inRange = createPolygon(
          null,
          useHandle ? getCursor2(this._orient) : null,
...
function installCommon2(registers) {
  ...
  registers.registerSubTypeDefaulter('visualMap', function (option) {
    return !option.categories && (!(option.pieces ? option.pieces.length > 0 : option.splitNumber > 0) || option.calculable)
      ? 'continuous'
      : 'piecewise';
  });
        ...
        function (v, visualType) {
          ...
          !exists &&
            each(this.stateList, function (state) {
              (option[state] || (option[state] = {}))[visualType] = visualDefault_default.get(
                visualType,
                state === 'inRange' ? 'active' : 'inactive',
                isCategory
              );
            });
        },
    ...
    PiecewiseModel2.prototype.getRepresentValue = function (piece) {
      ...
      if (this.isCategory()) {
        ...
        if (piece.value != null) {
          ...
          representValue =
            pieceInterval[0] === -Infinity && pieceInterval[1] === Infinity ? 0 : (pieceInterval[0] + pieceInterval[1]) / 2;
        }
      }
...
function ariaVisual(ecModel, api) {
  ...
  function setLabel() {
    ...
    if (seriesCnt < 1) {
      ...
      var seriesLabels_1 = [];
      var prefix =
        seriesCnt > 1 ? labelModel.get(['series', 'multiple', 'prefix']) : labelModel.get(['series', 'single', 'prefix']);
      ...
      ecModel.eachSeries(function (seriesModel, idx) {
        if (idx < displaySeriesCnt) {
          ...
          seriesLabel =
            seriesCnt > 1 ? labelModel.get(['series', 'multiple', withName]) : labelModel.get(['series', 'single', withName]);
        ...
        if (!dimInfo) {
          if (true) {
            errMsg = makePrintable(
              ...
              'Existing dimensions: ',
              upstream.cloneAllDimensionInfo(),
              '.\n',
              'Illegal condition:',
              exprOption,
              '.\n'
