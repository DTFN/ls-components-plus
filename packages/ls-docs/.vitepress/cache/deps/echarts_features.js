import { installLabelLayout } from './chunk-R4XCRTH7.js';
import './chunk-GI2ZEGHQ.js';
import {
  DataDiffer_default,
  Polygon_default,
    ...
    if (batchMany.length === 1 && !forceManyOne) {
      ...
      var separateAnimationCfg = defaults(
        {
          ...
          individualDelay:
            animationDelay &&
            function (idx, count2, fromPath, toPath) {
      ...
      for (var k = 0; k < count; k++) {
        ...
        animateOtherProps(
          fromIndividuals[k],
          toIndividuals[k],
          fromIsMany2 ? batchMany[k] : batch.one,
          fromIsMany2 ? batch.one : batchMany[k],
...
function querySeries(series, finder) {
  for (var i = 0; i < series.length; i++) {
    var found =
      (finder.seriesIndex != null && finder.seriesIndex === series[i].seriesIndex) ||
...
export { installLabelLayout as LabelLayout, installUniversalTransition as UniversalTransition };
