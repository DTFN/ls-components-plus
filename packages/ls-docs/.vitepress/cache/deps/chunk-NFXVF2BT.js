...
import { __export } from './chunk-SNAQBZPT.js';
...
function reformIntervals(list) {
  ...
  function littleThan(a, b, lg2) {
    return (
      a.interval[lg2] < b.interval[lg2] ||
      (a.interval[lg2] === b.interval[lg2] && (a.close[lg2] - b.close[lg2] === (!lg2 ? 1 : -1) || (!lg2 && littleThan(a, b, 1))))
    );
  }
}
  ...
  each(newCmptOptions, function (cmptOption) {
    ...
    while (
      ...
      (resultItem.newOption ||
        isComponentIdInternal(resultItem.existing) || // In mode "replaceMerge", here no not-mapped-non-internal-existing.
...
function makeIdAndName(mapResult) {
  ...
  each(mapResult, function (item, index) {
    ...
    keyInfo.name =
      opt.name != null ? makeComparableKey(opt.name) : existing ? existing.name : DUMMY_COMPONENT_NAME_PREFIX + index;
    if (existing) {
...
function handleGlobalMouseOverForHighDown(dispatcher, e2, api) {
  ...
  var _a2 = findComponentHighDownDispatchers(ecData.componentMainType, ecData.componentIndex, ecData.componentHighDownName, api),
...
function toggleSelectionFromPayload(seriesModel, payload, api) {
  ...
  seriesModel[
    payload.type === TOGGLE_SELECT_ACTION_TYPE ? 'toggleSelect' : payload.type === SELECT_ACTION_TYPE ? 'select' : 'unselect'
  ](dataIndex, dataType);
}
      ...
      if (ocrMax > e || icrMax > e) {
        ...
        if (arc < PI2) {
          ...
          if (it_1) {
            ...
            var y1 = yre - it_1[1];
            var a =
              1 / mathSin2(mathACos((x0 * x1 + y0 * y1) / (mathSqrt3(x0 * x0 + y0 * y0) * mathSqrt3(x1 * x1 + y1 * y1))) / 2);
...
function animateOrSetProps(animationType, el, props, animatableModel, dataIndex, cb, during) {
  ...
  var animationConfig = getAnimationConfig(
    ...
    animatableModel && animatableModel.getAnimationDelayParams ? animatableModel.getAnimationDelayParams(el, dataIndex) : null
  );
...
function createIcon(iconStr, opt, rect) {
  ...
  if (iconStr) {
    return iconStr.indexOf('image://') === 0
      ? ((style.image = iconStr.slice(8)), defaults(style, rect), new Image_default(innerOpts))
...
function animateLabelValue(textEl, dataIndex, data, animatableModel, labelFetcher) {
  ...
  textEl.percent = 0;
  (labelInnerStore.prevValue == null ? initProps : updateProps)(
    textEl,
    {
...
var localeStorage = {};
var localeModels = {};
var SYSTEM_LANG = !env_default.domSupported
  ? DEFAULT_LOCALE
  : (function () {
      var langStr =
...
// ../../node_modules/.pnpm/echarts@5.5.1/node_modules/echarts/lib/legacy/getTextRect.js
...
function positionElement(el, positionInfo, containerRect, margin, opt, out2) {
  ...
  if (boundingMode === 'raw') {
    rect =
      el.type === 'group'
        ? new BoundingRect_default(0, 0, +positionInfo.width || 0, +positionInfo.height || 0)
        : el.getBoundingRect();
  } else {
...
function copyLayoutParams(target, source) {
  source &&
    target &&
    each2(LOCATION_PARAMS, function (name) {
...
var rawSourceItemGetterMap =
  ...
  (_a[SOURCE_FORMAT_KEYED_COLUMNS] = function (rawData, startIndex, dimsDef, idx, out2) {
    var item = out2 || [];
...
var rawSourceValueGetterMap =
  ...
  (_c[SOURCE_FORMAT_OBJECT_ROWS] = function (dataItem, dimIndex, property) {
    return dataItem[property];
  }),
  (_c[SOURCE_FORMAT_KEYED_COLUMNS] = getRawValueSimply),
  (_c[SOURCE_FORMAT_ORIGINAL] = function (dataItem, dimIndex, property) {
    ...
    SourceManager2.prototype._createSource = function () {
      ...
      if (isSeries(sourceHost)) {
        ...
        var needsCreateSource =
          seriesLayoutBy !== upMetaRawOption.seriesLayoutBy || !!sourceHeader !== !!upMetaRawOption.sourceHeader || dimensions;
...
function buildSection(ctx, fragment, topMarginForOuterGap, toolTipTextStyle) {
  ...
  var subMarkupText =
    ctx.renderMode === 'richText'
      ...
      : wrapBlockHTML(subMarkupTextList.join(''), noHeader ? topMarginForOuterGap : gaps.html);
  if (noHeader) {
  ...
  return renderMode === 'richText'
    ...
    : wrapBlockHTML(
        (noMarker ? '' : markerStr) +
          ...
          (noValue ? '' : wrapInlineValueHTML(readableValueList, valueAlignRight, valueCloseToMarker, valueStyle)),
        topMarginForOuterGap
      );
}
...
function formatTooltipArrayValue(value, series, dataIndex, tooltipDims, colorStr) {
  ...
  var isValueMultipleLine = reduce(
    value,
    function (isValueMultipleLine2, val, idx) {
      var dimItem = data.getDimensionInfo(idx);
      return (isValueMultipleLine2 =
        isValueMultipleLine2 || (dimItem && dimItem.tooltip !== false && dimItem.displayName != null));
    },
    false
  );
  var inlineValues = [];
...
var SeriesModel =
  ...
  (function (_super) {
    ...
    SeriesModel2.prototype.isSelected = function (dataIndex, dataType) {
      ...
      var data = this.getData(dataType);
      return (
        (selectedMap === 'all' || selectedMap[getSelectionKey(data, dataIndex)]) &&
...
//# sourceMappingURL=chunk-NFXVF2BT.js.map
