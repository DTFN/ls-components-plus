import { setupDevtoolsPlugin } from './chunk-V4TPN3LZ.js';
import {
  computed,
  defineComponent,
  getCurrentInstance,
...
// ../../node_modules/.pnpm/vue-router@4.4.3_vue@3.4.38_typescript@5.5.4_/node_modules/vue-router/dist/vue-router.mjs
...
function createRouterMatcher(routes, globalOptions) {
  ...
  function resolve(location2, currentLocation) {
    ...
    if ('name' in location2 && location2.name) {
      ...
      if (true) {
        const invalidParams = Object.keys(location2.params || {}).filter(
          paramName => !matcher.keys.find(k => k.name === paramName)
        );
        if (invalidParams.length) {
          warn(
            `Discarded invalid param(s) "${invalidParams.join('", "')}" when navigating. See https://github.com/vuejs/router/blob/main/packages/router/CHANGELOG.md#414-2022-08-22 for more details.`
          );
        }
      }
...
function checkChildMissingNameWithEmptyPath(mainNormalizedRecord, parent) {
  if (parent && parent.record.name && !mainNormalizedRecord.name && !mainNormalizedRecord.path) {
    warn(
      `The route named "${String(parent.record.name)}" has a child without a name and an empty path. Using that name won't render the empty path child so you probably want to move the name to the child instead. If this is intentional, add a name to the child route to remove the warning.`
    );
  }
}
  ...
  return () =>
    new Promise((resolve, reject) => {
      ...
      const guardReturn = runWithContext(() =>
        guard.call(record && record.instances[name], to, from, true ? canOnlyBeCalledOnce(next, to, from) : next)
      );
    ...
    for (const name in record.components) {
      ...
      if (true) {
        if (!rawComponent || (typeof rawComponent !== 'object' && typeof rawComponent !== 'function')) {
          warn(
            `Component "${name}" in record with path "${record.path}" is not a valid component. Received "${String(rawComponent)}".`
          );
          ...
          rawComponent.__asyncLoader && // warn only once per component
    ...
    api => {
      ...
      function refreshRoutesView() {
        ...
        let routes = matcher.getRoutes().filter(
          route =>
            !route.parent || // these routes have a parent with no component which will not appear in the view
...
function markRouteRecordActive(route, currentRoute) {
  const isExactActive =
    currentRoute.matched.length && isSameRouteRecord(currentRoute.matched[currentRoute.matched.length - 1], route.record);
...
- Location:`,
    ...
    if (rawLocation.path != null) {
      if (
        ...
        !('name' in rawLocation) && // @ts-expect-error: the type is never
