...
async function getDestinationArray(doc: PDFDocumentProxy, dest: string | any[] | null): Promise<any[] | null> {
  return typeof dest === 'string' ? doc.getDestination(dest) : dest
}

async function getDestinationRef(doc: PDFDocumentProxy, destArray: any[] | null): Promise<RefProxy | null> {
...
const isSpecLike = (list: any[]): list is number[] => list && list.every(v => !Number.isNaN(v))
