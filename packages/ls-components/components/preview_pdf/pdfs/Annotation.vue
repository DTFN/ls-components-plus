...
import { EVENTS_TO_HANDLER, annotationEventsHandler } from './annotations';
...
async function render() {
  ...
  for (const evtHandler of EVENTS_TO_HANDLER) layer.value!.removeEventListener(evtHandler, annotationsEvents);
  ...
  const task = new AnnotationLayer(layerParameters).render(renderParameters);
  task.then(async () => {
    emit('annotationLoaded', (await getAnnotations())!);
  });

  for (const evtHandler of EVENTS_TO_HANDLER) layer.value!.addEventListener(evtHandler, annotationsEvents);
}
