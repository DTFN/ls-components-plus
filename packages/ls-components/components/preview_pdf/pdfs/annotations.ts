...
function getAnnotationsByKey(key: string, value: any, annotations: Object[]): any[] {
  ...
  if (annotations) {
    for (const annotation of annotations) {
      ...
      if (annotation[key as Key] === value) result.push(annotation);
    }
  }
  return result;
}
...
async function linkAnnotation(
  ...
  if (annotation.dest) {
    let explicitDest;
    if (typeof annotation.dest === 'string') explicitDest = await PDFDoc.getDestination(annotation.dest);
    else explicitDest = annotation.dest;
    ...
    if (Number.isInteger(destRef)) {
      ...
      console.warn(`Destination "${destRef}" is not a valid destination (dest="${annotation.dest}")`);
      return buildAnnotationData(INTERNAL_LINK, {
        referencedPage: null,
        offset: null
      });
...
function mergePopupArgs(annotation: HTMLElement) {
  const domList: any = annotation.getElementsByTagName('span');
  for (const spanElement of domList) {
...
function annotationEventsHandler(evt: Event, PDFDoc: PDFDocumentProxy, Annotations: Object[]) {
  ...
  if (annotation.tagName === 'DIV') annotation = annotation.firstChild! as HTMLElement;

  if (annotation.className === 'linkAnnotation' && evt.type === 'click') {
    const id: string | undefined = annotation.dataset?.annotationId;
    if (id) return linkAnnotation(getAnnotationsByKey('id', id, Annotations)[0] as LinkAnnotation, PDFDoc);
    ...
    if (id && evt.type === 'dblclick') return fileAnnotation(getAnnotationsByKey('id', id, Annotations)[0]);
    ...
    if (id) {
      const anno = getAnnotationsByKey('id', id, Annotations)[0];
      if (!anno.resetForm) {
        return inputAnnotation({ name: anno.fieldName, type: 'button' }, { actions: anno.actions, reset: false });
      } else {
        return inputAnnotation({ name: anno.fieldName, type: 'button' }, { actions: anno.actions, reset: true });
      }
    }
  }
}
