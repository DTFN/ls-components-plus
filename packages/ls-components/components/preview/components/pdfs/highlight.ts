...
function searchQuery(textContent: TextContent, query: string, options: HighlightOptions) {
  ...
  while ((match = regex.exec(textJoined)) !== null) matches.push([match.index, match[0].length, match[0]]);
  ...
  function appendHighlightDiv(idx: number, startOffset = -1, endOffset = -1) {
    ...
    if (startOffset > 0) {
      if (div.childNodes.length === 1 && div.childNodes[0].nodeType === Node.TEXT_NODE) {
        ...
        let alength = 0;
        const prevNodes = [];
        const nodeList: any = div.childNodes || [];
        for (const childNode of nodeList) {
          const textValue = childNode.nodeType === Node.TEXT_NODE ? childNode.nodeValue! : childNode.firstChild!.nodeValue!;
  ...
  for (const match of matches) {
    if (match.start.idx === match.end.idx) {
      ...
      for (let si = match.start.idx, ei = match.end.idx; si <= ei; si++) {
        ...
        else if (si === match.end.idx) appendHighlightDiv(si, -1, match.end.offset);
