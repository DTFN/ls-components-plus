...
import { getDestinationArray, getDestinationRef, getLocation, isSpecLike } from './components/pdfs/destination'
import { addStylesToIframe, createIframe } from './components/pdfs/miscellaneous'
...
export function usePDF(
  ...
  function processLoadingTask(source: any) {
    ...
    if (options.onPassword) {
      ...
      const onPassword: OnPasswordCallback = (updatePassword) => {
  ...
  async function print(dpi = 150, filename = 'filename') {
    ...
    const pagesNumbers = [...new Array(savedDocument.numPages).keys()].map(val => val + 1)
