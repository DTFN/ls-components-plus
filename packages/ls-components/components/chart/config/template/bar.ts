import type { ChartDataType, ChartMapDataType, ChartTemplatePatchType } from '@cpo/chart/types'
/**
 * 柱状图模板配置生成器。
 *
...
function setTooltipFormat(data: any, legend: boolean, legendIcon: string | undefined, i: number, defBarColor: string, tooltipValueFormatter: any | undefined, dataIndex: number) {
...
function getPos(axis: string, labelPosition: 'both' | 'insideBoth', status: boolean) {
...
function setSeriesOption(templatePatch: ChartTemplatePatchType, item: any, index: number) {
  const {
    type = 'simple',
    showBarFont = true,
