...
import { blobUrl, validateFileType } from '@/utils/utils'
...
watch(() => propsUploadFiles.value, (newVal) => {
  if (!isEqual(newVal, uploadFiles.value))
    fileList.value = newVal
}, { immediate: true, deep: true })

watch(() => uploadFiles.value, (newVal) => {
  if (!isEqual(newVal, propsUploadFiles.value))
...
const handleSuccess: UploadProps['onSuccess'] = (_response: any, _uploadFile: UploadFile, _uploadFiles: UploadFiles) => {
  ...
  if (_response && _response.code === 200) {
    if (props.isDealRes)
      isSuc = !!(_response.data && _response.data.filePathKey)
  }
  else {
    isSuc = false
  }
  ...
  if (isSuc) {
    if (props.showSuccessMessage)
      ElMessage.success(props.successMessage || '上传成功')
...
// 文件上传时的钩子
const handleProgress: UploadProps['onProgress'] = (_evt: UploadProgressEvent, _uploadFile: UploadFile, _uploadFiles: UploadFiles) => {
...
const beforeUpload: UploadProps['beforeUpload'] = (rawFile: UploadRawFile) => {
  ...
  if (accept)
    isAccord = validateFileType(accept, rawFile)
  ...
  if (props.size)
    isLtM = rawFile.size / 1024 / 1024 < props.size
