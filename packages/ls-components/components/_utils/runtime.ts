...
import { hasOwn, isObject } from '@cpo/_utils/check'
import { fromPairs } from 'lodash-unified'
import { warn } from 'vue'
import { epPropKey } from './types'
...
>(prop: EpPropInput<Type, Value, Validator, Default, Required>, key?: string): EpPropFinalized<Type, Value, Validator, Default, Required> {
  // filter native prop type and nested prop, e.g `null`, `undefined` (from `buildProps`)
  if (!isObject(prop) || isEpProp(prop))
    return prop as any

  const { values, required, default: defaultValue, type, validator }: any = prop
...
>(props: Props): {
  ...
  return fromPairs(Object.entries(props).map(([key, option]) => [key, buildProp(option as any, key)])) as any
}
