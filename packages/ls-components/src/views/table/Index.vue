...
<template>
  <div class="demo-wrap">
    ...
    <h2 class="demo-title">
      高级功能：排序 / 筛选 / 展开 / 合并 / 单选 / 分页 / 自定义表头 / 自定义列内容 / 自定义筛选图标
    </h2>
    <LSTable
    ...
    >
      ...
      <template #operate="{ row }">
        <el-button link type="primary" @click="ElMessage.info(`查看订单：${row.orderNo}`)">
          查看
        </el-button>
        <el-button link type="danger" @click="ElMessage.warning(`删除订单：${row.orderNo}`)">
          删除
        </el-button>
      </template>
    ...
    <div class="test-toolbar">
      <el-button size="small" @click="emptyVariantData = [...allData]">
        恢复数据
      </el-button>
      <el-button size="small" @click="emptyVariantData = []">
        清空数据（默认空状态）
      </el-button>
    </div>
    ...
    <!-- ────────── Expose 方法 + 行事件 ────────── -->
    <h2 class="demo-title">
      Expose 方法调用 + 行事件（row-click / cell-click）
    </h2>
    <p class="test-label">
      通过 ref 访问 TableRef 调用 el-table 原生方法；@row-click 通过 attrs 直接绑定
    </p>
    <div class="test-toolbar">
      <el-button size="small" type="primary" @click="exposeTableRef?.TableRef?.clearSelection()">
        clearSelection
      </el-button>
      <el-button size="small" @click="exposeTableRef?.TableRef?.toggleAllSelection()">
        toggleAllSelection
      </el-button>
      <el-button size="small" @click="exposeTableRef?.TableRef?.doLayout()">
        doLayout
      </el-button>
      <el-button size="small" @click="exposeTableRef?.TableRef?.clearSort()">
        clearSort
      </el-button>
    </div>
    <p class="test-label" style="color: #409eff">
      行点击日志：{{ rowClickLog }}
    </p>
    <LSTable
      ref="exposeTableRef"
